                    ...
                    if (e.LogicalName.Equals("dc_mealfood", StringComparison.OrdinalIgnoreCase))
                    {
                        try
                        {
                            ...
                            */
                            ...
                            if (useMinAmount || portionSize < minAmount)
                            {
                                multiplier = minAmount/portionSize;

                                
                            }
                            else if (portionSize > minAmount && moveTo > 0)
                            {
                                decimal targetPortionSize = minAmount;
                                while (true)
                                {
                                    if (targetPortionSize > portionSize)
                                    {
                                        break;
                                    }
                                    else
                                    {
                                        targetPortionSize += moveTo;
                                    }
                                }

                                multiplier = targetPortionSize / portionSize;
                            }

                            logger.debug("portionSize: " + portionSize);
                            logger.debug("Setting portion size:" + (decimal)e["dc_portionsize"] * multiplier);
                            ...
                            e["dc_alcohol"]         = (decimal) e["dc_alcohol"]*multiplier;
