...
namespace DynamicConnections.NutriStyle.MenuGenerator.Engine.Helpers
{
    public class General
    {
        ...
        public static void DataGrid_AttachEvent(DataGrid dg, DataGridRowEventArgs e, RoutedEventHandler action, Type type)
        {
            ...
            try
            {
                foreach (DataGridColumn c in dg.Columns)
                {
                    if (c.GetCellContent(e.Row).GetType() == type)
                    {
                        if (type == typeof (CheckBox))
                        {
                            CheckBox ccb = (CheckBox) c.GetCellContent(e.Row);
                            ccb.Click -= action;
                            ccb.Click += new RoutedEventHandler(action);
                        }
                    }
