        ...
        public static List<MealFoodWrapper> Execute(Entity meal, List<MealFoodWrapper> mealFoods, Dictionary<Guid, List<MealFoodWrapper>> foodComponents)
        {
            ...
            try
            {
                ...
                for (int x = 0; x < count; x++)
                {
                    ...
                    if (!mfw.IsFavorite)
                    {
                        ...
                        for (int y = 5; y < 35; y = y + 5)
                        {
                            ...
                            if (substitute != null)
                            {
                                ...
                                {
                                    logger.debug("Substitution worked.  Keeping it:" + postSubScore + ":" + preSubScore +":"+ (decimal)meal["dc_kcals_actuals"]);
                        ...
                        if (!foodComponents.ContainsKey(mfw.Category))
                        {
                            logger.error("foodComponents doesn't contain key: " + mfw.Category);
                        }
                        ...
                        foreach (MealFoodWrapper mealFoodWrapper in list)
                        {
                            ...
                            if (PerformRatioGapAnalysis.Execute(meal, mealFoods))
                            {
                                logger.debug("SubstituteFood1: Generated a meal within acceptable parameters.  Moving to next meal: "+(decimal)meal["dc_kcals_actuals"]);
                                return (mealFoods);
                            }
                            else
                            {
                                ...
                                {
                                    logger.debug("Substitution worked.  Keeping it.  Removed " + foodName + ".  Added: " + mealFoodWrapper.Name + ":" + newSubScore + " : " + (decimal)meal["dc_kcals_actuals"]);
                    ...
                    preSubScore = Meal.Score(meal);
                    logger.debug("MEAL SCORE (POST): " + preSubScore + " : " + (decimal)meal["dc_kcals_actuals"]);
                }
                return (mealFoods);
            }
            catch (Exception ex)
