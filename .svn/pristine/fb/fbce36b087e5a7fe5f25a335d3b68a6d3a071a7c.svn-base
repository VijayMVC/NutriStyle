...
namespace DynamicConnections.NutriStyle.CRM2011.WebServices.Engine
{
    public class User
    {
        ...
        /// <summary>
        /// Retrieve the shopping list for the primary menu
        /// </summary>
        /// <param name="contactId"></param>
        /// <returns></returns>
        public Guid RetrievePrimaryShoppingListId(Guid contactId)
        {
            ...
            try
            {
                String fetchXml = @"<fetch version='1.0' output-format='xml-platform' mapping='logical' distinct='false'>
                  <entity name='dc_shoppinglist'>
                    ...
                    <link-entity name='dc_menu' from='dc_menuid' to='dc_menuid' alias='aa'>
                      <filter type='and'>
                ...
                logger.debug("Found " + response.Entities.Count + " shoppinglist records");
                if (response != null && response.Entities.Count > 0)
                {
                    Entity shoppingList = (Entity)response.Entities[0];
                    shoppingListId = shoppingList.Id;
                    logger.debug("shoppingListId: " + shoppingListId);
                }
            }
            catch (FaultException<Microsoft.Xrm.Sdk.OrganizationServiceFault> ex)
            {
        ...
        public Guid RetrievePrimaryMenuId(Guid contactId)
        {
